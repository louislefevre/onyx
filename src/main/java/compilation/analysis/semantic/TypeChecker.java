 ...
 * <p>
 * It creates an AnnotatedParseTree that mirrors the same structure of the ParseTree generated by the Parser, except
 * Statements are given extra information concerning the data types used (the word 'Annotated' being used to refer to
 * this). During this stage any attempt to perform operations on values of incompatible data types are identified
 * and reported as an error.
 * <p>
 * The ParseTree produced by the Parser is retrieved, with its contents being parsed recursively to mirror the
 * structure of the tree. The Expressions contained within each individual Statement are inspected in an attempt to
 * discover whether or not operations being performed on unary values or between binary values are valid, based on
 * their data type(s).
 *
...
public final class TypeChecker
{
     ...
     * <p>
     * The TypeChecker adds any errors to the errorHandler, and adds any declared Symbols to the symbolTable.
     *
     * @param parser The Parser used to generate the ParseTree
     ...
     *
     * @return An AnnotatedParseTree containing the root AnnotatedStatement
     * @throws Exception If the ParseTree contains a StatementType that has not been defined
     */
